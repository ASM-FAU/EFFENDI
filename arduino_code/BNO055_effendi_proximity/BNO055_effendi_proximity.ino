...
Adafruit_BNO055 bno = Adafruit_BNO055(71, 0x28);
...
unsigned long prevMillis = 0; // will store last time the motor state was updated
unsigned long curMillis = 0;

unsigned long prevMillisProx = 0; // will store last time the proximity was updated
unsigned long curMillisProx = 0;
...
void loop(void) 
{
  ...
  curMillis = millis();
  float frequency = 1000.0 / (curMillis - prevMillis);
  ...
  // Proximity sensor can only do 8.4Hz so check every 8 Hz
  if (millis() - prevMillisProx > 4) {
    //ambient = vcnl.readAmbient();
    proximity = vcnl.readProximity();
    prevMillisProx = millis();
  }
...
//  oldDataPitch = pitch_filtered;
  ...
  Serial.println(proximity); //Click Value //coordinate[3]
  //Serial.print(",");
